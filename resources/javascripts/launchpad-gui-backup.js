$(document).ready(function() { 
	...
	var initPinDiv = function(board){
		...
		$pinDiv.append('<div id="pin1to10" class="col-sm-1 pin-col"></div>');
		$pinDiv.append('<div id="pin21to30" class="col-sm-1 pin-col"></div>');
		$pinDiv.append('<div id="pin31to40" class="col-sm-offset-8 col-sm-1 pin-col"></div>');
		$pinDiv.append('<div id="pin11to20" class="col-sm-1 pin-col"></div>');
		...
		$.each(board.pins, function(index, pin){
			$pinButton = $('<button></button>').html(index).attr(
				{'id':'pinBtn'+index, 'class':'btn btn-default btn-block',
			...
			$selectDropdown = $('<select></select>').attr({'name':'select'+index,'class':'form-control input-sm'});
			...
			$(document).on('change','select[name="select' +index + '"]',function(){
			...
			if ($('#pinBtn'+index).position()){
				console.log(index + ': ' + $('#pinBtn'+index).position().left);
			}
				


			if (pin.mode == board.MODES.INPUT){
				$pinButton.attr('style','background:orange');
			}
			else if (pin.mode == board.MODES.OUTPUT){
				$pinButton.attr('style','background:#2DCC30'); //green
			}
			else if (pin.mode == board.MODES.ANALOG){
				$pinButton.attr('style','background:#44DBFC'); //blue
			}
			else if (pin.mode == board.MODES.PWM){
				$pinButton.attr('style','background:purple');
			}
			else if (pin.mode == board.MODES.SERVO){
				$pinButton.attr('style','background:purple');
			}
			else {
				$pinButton.attr('style','background:grey');	
			}
		});
