$(document).ready(function() { 
	...
	$connectBtn.click(function(){
		...
		launchpad.connect(selectedPort, function(board){
			...
			$('#leftPanel').html(''); 			// empty existing pins
			initializeLeftPanel(board);			//initialize pin layout
	...
	// attach analog read events
	launchpad.analogRead(function(data){
		//console.log('analogRead: pin' + data.pin + ' - ' + data.value);
		$('#pin'+data.pin+' .pin-analog').html(data.value);
	});

	// attach digital read events
	launchpad.digitalRead(function(data){
		//console.log('digitalRead: pin' + data.pin + ' - ' + data.value);
		$('#pin'+data.pin+' .pin-input').html(data.value?'HIGH':'LOW');
	});


	// Initialize #leftPanel on client
	var initializeLeftPanel = function(board){
		$leftPanel = $('#leftPanel');
